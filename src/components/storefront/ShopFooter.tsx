    ...
    <footer className="mt-16 border-t border-border" style={themeBg}>
      <div className="container py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
            ...
            {shop.contact.socialLinks.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-3">
                {shop.contact.socialLinks.map((s) => (
                  <a key={s.platform} href={s.url} target="_blank" rel="noopener noreferrer" className="text-xs font-medium px-2.5 py-1 rounded-full bg-card border border-border hover:border-primary/50 hover:text-primary transition-colors">
                    {s.platform}
                  </a>
                ))}
              </div>
          ...
          <div>
            <h3 className="font-heading font-bold text-sm text-foreground mb-3 uppercase tracking-wide">Liên hệ</h3>
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li className="flex items-start gap-2">
                <Phone className="w-4 h-4 mt-0.5 flex-shrink-0 text-primary" />
                <a href={"tel:" + shop.contact.phone.replace(/\s+/g, "")} className="hover:text-foreground">{shop.contact.phone}</a>
              </li>
              <li className="flex items-start gap-2">
                <Mail className="w-4 h-4 mt-0.5 flex-shrink-0 text-primary" />
                <a href={"mailto:" + shop.contact.email} className="hover:text-foreground">{shop.contact.email}</a>
              </li>
              <li className="flex items-start gap-2">
          ...
          <div>
            <h3 className="font-heading font-bold text-sm text-foreground mb-3 uppercase tracking-wide">Thông tin pháp lý</h3>
            {info ? (
              <ul className="space-y-2 text-sm text-muted-foreground">
                <li className="flex items-start gap-2">
                ...
                <li className="flex items-start gap-2">
                  <FileCheck2 className="w-4 h-4 mt-0.5 flex-shrink-0 text-primary" />
                  <span>Mã ĐKKD: <span className="font-medium text-foreground">{info.registrationNumber}</span></span>
                </li>
                ...
                {info.ownerName && (
                  <li className="flex items-start gap-2">
                    <UserIcon className="w-4 h-4 mt-0.5 flex-shrink-0 text-primary" />
                    <span>Đại diện: <span className="font-medium text-foreground">{info.ownerName}</span></span>
                  </li>
                )}
                {info.note && (
                  <li className="text-xs text-muted-foreground/80 pt-1 italic">{info.note}</li>
                )}
              </ul>
            ) : (
              <p className="text-xs text-muted-foreground italic">Chưa cập nhật thông tin pháp lý.</p>
            )}
          </div>
        </div>

        <div className="mt-8 pt-5 border-t border-border/60 text-xs text-muted-foreground text-center">
