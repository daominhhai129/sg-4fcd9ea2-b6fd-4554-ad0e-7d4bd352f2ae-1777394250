...
export default function SubAdminPage() {
  ...
  const handleCreateClick = () => {
    if (atCap) {
      toast({ title: t("sub.toastCapReached"), description: t("sub.toastCapReachedDesc").replace("{n}", cap.toLocaleString("vi-VN")), variant: "destructive" });
      return;
    }
    setCreateOpen(true);
  };
  ...
  const handleResetPassword = (userId: string, name: string) => {
    ...
    toast({ title: t("sub.toastResetPwd"), description: t("sub.toastResetPwdDesc").replace("{name}", name) });
  };

  return (
    <>
      <SEO title={t("sub.title")} />
      <div className="min-h-screen bg-background">
        <header className="sticky top-0 z-30 h-16 bg-card/90 backdrop-blur-lg border-b border-border/50 flex items-center px-4 lg:px-8">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <Button variant="outline" size="sm" className="rounded-xl" onClick={logout}>
              <LogOut className="w-4 h-4 mr-1.5" /> {t("sub.logout")}
            </Button>
          </div>
        </header>
        ...
        <main className="p-4 lg:p-8 space-y-6 max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="rounded-2xl bg-card border border-border/50 p-5">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm text-muted-foreground">{t("sub.statManaged")}</span>
            ...
            <div className="rounded-2xl bg-card border border-border/50 p-5">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm text-muted-foreground">{t("sub.statActive")}</span>
            ...
            <div className="rounded-2xl bg-card border border-border/50 p-5">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm text-muted-foreground">{t("sub.statRemaining")}</span>
          ...
          {atCap && (
            <div className="rounded-2xl bg-destructive/10 border border-destructive/30 p-4 flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-destructive">{t("sub.capReachedTitle").replace("{n}", cap.toLocaleString("vi-VN"))}</p>
                <p className="text-sm text-muted-foreground mt-0.5">{t("sub.capReachedDesc")}</p>
              </div>
            </div>
          )}
          ...
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <h2 className="text-lg font-heading font-bold text-foreground">{t("sub.myShopsTitle")}</h2>
              <div className="flex gap-2">
                <div className="relative flex-1 sm:w-72">
                  <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
                  <Input placeholder={t("sub.searchPh")} value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 rounded-xl" />
                </div>
                <Button className="gradient-primary text-white border-0 rounded-xl" onClick={handleCreateClick} disabled={atCap}>
                  <UserPlus className="w-4 h-4 mr-1.5" /> {t("sub.createShop")}
                </Button>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-border/50 overflow-hidden bg-card">
              <table className="w-full">
                ...
                <tbody>
                  ...
                  {filtered.length === 0 && (
                    <tr><td colSpan={8} className="px-4 py-12 text-center text-sm text-muted-foreground">{t("sub.emptyShops")}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <EditUserDialog
          ...
          onSave={(input) => { if (editingUser) { updateUser(editingUser, input); toast({ title: t("sub.toastUserUpdated") }); } }}
        />
        <DomainDialog
          open={!!domainUser}
          ...
          onSave={(domain) => { if (domainUser) { setUserDomain(domainUser, domain); toast({ title: t("sub.toastDomainUpdated") }); } }}
        />
        <LimitDialog
          open={!!limitsUser}
          ...
          onSave={(value) => { if (limitsUserData?.shopId) { setShopLimit(limitsUserData.shopId, value); toast({ title: t("sub.toastLimitUpdated") }); } }}
        />

        <Dialog open={profileOpen} onOpenChange={setProfileOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle className="font-heading">{t("sub.profileTitle")}</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div><Label className="text-sm font-semibold">{t("sub.fullName")}</Label><Input value={profileForm.name} onChange={(e) => setProfileForm({ ...profileForm, name: e.target.value })} className="rounded-xl mt-1.5" /></div>
              <div><Label className="text-sm font-semibold">{t("sub.email")}</Label><Input type="email" value={profileForm.email} onChange={(e) => setProfileForm({ ...profileForm, email: e.target.value })} className="rounded-xl mt-1.5" /></div>
              <div><Label className="text-sm font-semibold">{t("sub.phone")}</Label><Input value={profileForm.phone} onChange={(e) => setProfileForm({ ...profileForm, phone: e.target.value })} className="rounded-xl mt-1.5" /></div>
              <div>
                <Label className="text-sm font-semibold">{t("sub.newPwd")}</Label>
                <Input type="text" value={profileForm.password} onChange={(e) => setProfileForm({ ...profileForm, password: e.target.value })} className="rounded-xl mt-1.5 font-mono" placeholder={t("sub.newPwdPh")} />
                <p className="text-xs text-muted-foreground mt-1.5">{t("sub.newPwdHint")}</p>
              </div>
              <div className="flex gap-3 pt-2 border-t">
                <Button variant="outline" className="flex-1 rounded-xl" onClick={() => setProfileOpen(false)}>{t("sub.cancel")}</Button>
                <Button
                  ...
                  onClick={() => {
                    ...
                    toast({ title: t("sub.toastProfileSaved") });
